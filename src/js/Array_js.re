...
[@bs.send] external reducei: (t('a), ('b, 'a, int) => 'b, 'b) => 'b = "reduce";
...
[@bs.send] external removeCountInPlace: (t('a), ~pos:int, ~count:int) => t('a) = "splice";

let remove = (arr: t('a), pos: int) =>
  removeInPlace(copy(arr), pos);
...
module Js = {
  ...
  [@bs.scope ("Array", "prototype", "push")]
    [@bs.val] external pushMany: (t('a), t('a)) => unit = "apply";
  [@bs.scope ("Array", "prototype", "unshift")]
    [@bs.val] external unshiftMany: (t('a), t('a)) => unit = "apply";

  [@bs.send] external spliceInPlace: (t('a), ~pos:int, ~remove:int, ~add:t('a)) => t('a) = "splice";
